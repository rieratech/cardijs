...
class Message{
    constructor(to,gameIsLive,gameRound,currentPlayer,cardiAt,requestedSuit,cardOnTable,cardIssued,reward,comboReward,recycle,playRejected){
        this.to = to,
        ...
        this.playRejected = playRejected
    }
}
class Player{
...
class Cards{
    ...
    match(attempt, liveCard, continuousPlay = false, punish = false){
        if(liveCard.rank === "X") return attempt.suit === liveCard.suit || this.isInGroup(this.masterRanks, attempt.rank);
...
class Game{
    ...
    sendMessage(cardIssued = {}, reward = 0, comboReward = 0, recycle = false, playRejected = false){
        let message = new Message (
            ...
            this.getCurrentPlayer().cardiAt,
            this.requestedSuit,
            this.cards.table[this.cards.table.length - 1],
            cardIssued,
            reward,
            comboReward,
            recycle,
    ...
    mastersRequest(mastersRequest){
        if(!this.currentPlayer.isMaster) return this.sendMessage({},0,0,false,true);

        this.requestedSuit = mastersRequest;
        return this.goToNext();
    }
    ...
    play(playerId, attempt){
        if(!this.canPlay(playerId)) return this.sendMessage();
        if(attempt.goToNext)return this.goToNext();
        if(attempt.callCardi) return this.callCardi();
        if(attempt.mastersRequest.length > 0) return this.mastersRequest(attempt.mastersRequest);
        if((this.getCurrentPlayer().cards.length - 1) < attempt.index || attempt.index < 0) return this.sendMessage();
        ...
        if(!this.cards.match(attemptCard, liveCard, continuousPlay, punish)) return this.sendMessage({},0,0,false,true);
        ...
        return this.sendMessage({}, reward, continuousPlayReward);
    }
}